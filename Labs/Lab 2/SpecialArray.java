 ...
 * 
 * To solve the second problem I use various for loops to iterate through each side of the 2D array in a clockwise order. I used the 
 * StringBuilder class to append elements because the process of concatenation can be slow. I wrote this code with the spiral extra
 * credit problem in mind. My thought was if I could navigate the perimeter of the 2D array I could increment the start and stop
...
public class SpecialArray {
	...
	private static int[] findIndex(int[] nums, int target, int start) {
		...
		return findIndex(nums, target, start + 1);
	}
	...
	private static String getSpiral(int[][] arr, int row, int col, boolean[][] visited, StringBuilder result, int direction) {
		...
		// switch statement will call getSpiral again with updated parameters.
		// will keep moving in current direction, until and index does not exist or is marked as visited.
		// at this point the program will check the index to it`s respective left and if valid, proceed
		switch (direction) {
		
		case 0:   // go left (col + 1). If returned, set direction "down" and move down (row + 1)
			result.append(arr[row][col]).append(" ");
			getSpiral(arr, row, col + 1, visited, result, 0);
		    getSpiral(arr, row + 1, col, visited, result, 1);
		    break;
	
		case 1:   // move down, if returned, set direction to right and move right (col - 1)
			result.append(arr[row][col]).append(" ");
			getSpiral(arr, row + 1, col, visited, result, 1);
		    getSpiral(arr, row, col - 1, visited, result, 2);
		    break;
	
		case 2:   // move right, if returned set direction to up, move up (row -1)
			result.append(arr[row][col]).append(" ");
			getSpiral(arr, row, col - 1, visited, result, 2);
		    getSpiral(arr, row - 1, col, visited, result, 3);
		    break;
		
		case 3:   // move up, if returned set direction to left and move left
			result.append(arr[row][col]).append(" ");
			getSpiral(arr, row - 1, col, visited, result, 3);
		    getSpiral(arr, row, col + 1, visited, result, 0);
		    break;
